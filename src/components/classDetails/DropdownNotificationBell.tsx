...
import { approveJoinRequest, getJoinRequests, rejectJoinRequest } from "@/services/classService";
...
export default function TeacherNotificationBell({ teacherId }: { teacherId: number }) {
  const [notifications, setNotifications] = useState<JoinRequestDTO[]>([]);

  useEffect(() => {
    if (!teacherId) return;

    // Lấy tất cả thông báo cũ khi component mount
...
const handleRequestAction = async (requestId: number, action: 'accept' | 'reject', reason?: string) => {
    try {
      if (action === 'accept') {
      ...
      // Sau khi xử lý thành công trên server, cập nhật state trên client
      setNotifications(notifications.filter(req => req.requestId !== requestId));
...
<div className="relative">
  <DropdownMenu>
    ...
    <DropdownMenuContent className="w-96 p-2 space-y-2">
      <DropdownMenuLabel className="font-bold text-lg text-green-700">Thông báo mới</DropdownMenuLabel>
      <DropdownMenuSeparator />
      {notifications.length === 0 ? (
        ...
        notifications.map((n, index) => (
          <DropdownMenuItem key={index} className="flex items-start justify-between space-x-2 p-2 rounded-md hover:bg-gray-100 cursor-pointer">
            <div className="flex items-start flex-1 space-x-2">
              <Users className="w-5 h-5 text-green-700 mt-1" />
              <div className="flex-1">
                <p className="font-medium text-sm">Yêu cầu tham gia lớp học</p>
                <p className="text-sm text-gray-600">Sinh viên: {n.studentName}</p>
